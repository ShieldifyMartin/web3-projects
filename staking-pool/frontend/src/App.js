...
function App() {
  ...
  const invest = async (e) => {
    e.preventDefault();
    await stakingPool.invest({ value: e.target.elements[0].value });
    const amountInvested = await stakingPool.balances(signerAddress);
    setAmountInvested(amountInvested);
  };
        ...
        <div className="col-sm-12">
          ...
          <div className="jumbotron">
            <h1 className="display-4 text-center">
              Become a validator in ETH 2.0 and earn between 5 to 20pct per year
            </h1>
            <p className="lead text-center">
              Your investment is pooled with other people. You can invest any
              amount, no need for 32 ETH normally required to run a validator by
              yourself.
            </p>
          </div>
        </div>
