    ...
    Ownable
{
    ...
    VRFCoordinatorV2Interface
        private immutable i_vrfCoordinator;
    uint64 private immutable i_subscriptionId;
    bytes32 private immutable i_gasLane;
    uint32 private immutable i_callbackGasLimit;
    uint16
        private constant REQUEST_CONFIRMATIONS =
        3;
    ...
    uint256 internal constant MAX_CHANCE_VALUE =
        100;
    string[] internal i_nftTokenUries;

    mapping(uint256 => address)
        public s_requestIdSenders;
        ...
        returns (uint256 requestId)
    {
        ...
        s_requestIdSenders[requestId] = msg
            .sender;
    ...
    function fulfillRandomWords(
        ...
        address requestSender = s_requestIdSenders[
                requestId
            ];
        uint256 moddedRng = randomWords[0] %
            MAX_CHANCE_VALUE;
        Breed dogBreed = getBreedFromModdedRng(
            moddedRng
        );
    ...
    function getBreedFromModdedRng(
        ...
        uint256[3]
            memory chanceArray = getChanceArray();
        for (
            uint256 i = 0;
            i < chanceArray.length;
            i++
        ) {
            if (
